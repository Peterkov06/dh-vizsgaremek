...
namespace backend.Data.Configurations.CoursesBase
{
    public class CourseToTagConfiguration : IEntityTypeConfiguration<CourseToTag>
    {
        public void Configure(EntityTypeBuilder<CourseToTag> builder)
        {
            ...
            builder.HasOne(x => x.Course).WithMany(x => x.CourseToTags).HasForeignKey(x => x.CourseId).IsRequired().OnDelete(DeleteBehavior.Cascade);
            builder.HasOne(x => x.Tag).WithMany().HasForeignKey(x => x.TagId).IsRequired().OnDelete(DeleteBehavior.Cascade);
        }
    }
}
