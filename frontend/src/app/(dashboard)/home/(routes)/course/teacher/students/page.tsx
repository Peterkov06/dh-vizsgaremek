...
const StudentsPage = () => {
  ...
  const [myStudents, setMyStudents] = useState<StudentManagementData>();

  const HandleFetches = async () => {
    await fetchWithAuth(`/api/pages/teacher/courses/${courseId}/`)
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
        setMyStudents(data);
  ...
  function HandleRedirect(
    ...
    redirect(
      `/home/students/student/${name}?studentId=${studentId}&wallId=${wallId}&chatId=${chatId}`,
    );
  }
      ...
      <section className="border-4 border-light-bg-gray p-3 rounded-2xl w-fit">
        ...
        <div className="max-w-[80em] max-h-[12em]">
          <div className="flex-col lg:flex-row max-h-[12em] flex gap-3 lg:gap-7 overflow-auto touch-pan-y">
            {myStudents?.pendingEnrollments?.map((ps) => (
              <div
                key={ps.userId}
              ...
              >
                <Avatar className="size-14 lg:size-20 bg-background">
                  <AvatarImage
                    src={ps.profilePictureUrl || "/defaults/default_avatar.jpg"}
                  ></AvatarImage>
                </Avatar>
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                            ...
                            onClick={() => {
                              HandleAcceptance(ps.userId, true);
                            }}
                          >
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                            ...
                            onClick={() => {
                              HandleAcceptance(ps.userId, false);
                            }}
                          >
